...
(() => {
	...
	const TEACHERS = [
		...
		},
		{
			name: "Роман Федоряка",
			about:
				"Викладач курсу «Тригерні точки та міофасциальний больовий синдром».Лікар ортопед-травматолог. Досвід понад 15 років.",
			photo: "./assets/img/roman_f.png",
			courses: ["Тригерні точки та міофасціальний больовий синдром"],
			collage: [
				"./assets/img/teachers_teacher-3.jpg",
				"./assets/img/teachers_teacher-2.jpg",
				"./assets/img/teachers_teacher-4.jpg",
...
document.addEventListener('DOMContentLoaded', function () {
	const reviewsSliderEl = document.querySelector('.course-reviews__slider');

	if (!reviewsSliderEl || typeof Swiper === 'undefined') return;

	const reviewsSection = reviewsSliderEl.closest('.course-reviews');
	const prevBtn = reviewsSection.querySelector('.course-reviews__btn--prev');
	const nextBtn = reviewsSection.querySelector('.course-reviews__btn--next');
	const paginationEl = reviewsSection.querySelector('.course-reviews__pagination');
...
////
// Только один checkbox в пределах одной карточки .master-card
document.querySelectorAll('[data-master-card]').forEach((card) => {
	const checkboxes = card.querySelectorAll('.master-opt__input[type="checkbox"]');

	if (!checkboxes.length) return;

	// если по ошибке в HTML отмечено несколько — оставляем только первый
	let firstCheckedFound = false;

	checkboxes.forEach((checkbox) => {
		if (checkbox.checked && !firstCheckedFound) {
			firstCheckedFound = true;
		} else if (checkbox.checked && firstCheckedFound) {
	...
	checkboxes.forEach((checkbox) => {
		checkbox.addEventListener('change', function () {
			...
			// Если нужно, чтобы хотя бы один checkbox ВСЕГДА был выбран,
